...
int main() {
    ...
    for (int limit = 1e3; limit < 1e8; limit *= 10) {
        ...
        std::cout << "SET  insert at size " << limit << ": " << s_time << " milliseconds per M items." << std::endl;

        auto us_time = time_set(us_numbers, 1);
        std::cout << "USET insert at size " << limit << ": " << us_time << " milliseconds per M items." << std::endl;
