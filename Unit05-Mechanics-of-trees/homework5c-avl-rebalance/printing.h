...
int tree_height(const Node* node) {
    if (node == nullptr) {
        return 0;
    }
...
void pretty_print_tree(const Node* root) {
    if (root == nullptr) {
        std::cout << "Empty tree" << std::endl;
        return;
    }

    std::queue<const Node*> traversal;
    traversal.push(root);

    int height = tree_height(root);
    for (int level = 0; level < height; ++level) {
        int spaces_in_between = num_spaces_for_level(height, level);
        int spaces_before = (spaces_in_between - 3) / 2;
        ...
        for (int ni = 0; ni < nodes; ++ni) {
            ...
            const Node* node = traversal.front();
            traversal.pop();
...
// numbers.
void ugly_print_tree(const Node* root) {
    if (root == nullptr) {
        std::cout << "Empty tree" << std::endl;
    } else {
