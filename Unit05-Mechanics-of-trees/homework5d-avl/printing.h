...
void ugly_print_tree_helper(const AVL<int>::Node* node) {
    if (node == nullptr) {
        return;
    }
    const AVL<int>::Node* left = node->left;
    const AVL<int>::Node* right = node->right;
    std::cout << node->value << '|' << node->height << ": ";
    if (left == nullptr) {
        std::cout << "____";
    } else {
        std::cout << left->value << '|' << left->height;
    }
    std::cout << ' ';
    if (right == nullptr) {
        std::cout << "____";
    } else {
        std::cout << right->value << '|' << right->height;
    }
    std::cout << std::endl;
...
int tree_height(const AVL<int>::Node* node) {
    if (node == nullptr) {
        return 0;
    }
...
void pretty_print_tree(const AVL<int>& tree) {
    const AVL<int>::Node* root = tree.getRootNode();
    if (root == nullptr) {
        std::cout << "Empty tree" << std::endl;
        return;
    }

    std::queue<const AVL<int>::Node*> traversal;
    traversal.push(root);

    int height = tree_height(root);
    for (int level = 0; level < height; ++level) {
        int spaces_in_between = num_spaces_for_level(height, level);
        int spaces_before = (spaces_in_between - 4) / 2;
        ...
        for (int ni = 0; ni < nodes; ++ni) {
            ...
            const AVL<int>::Node* node = traversal.front();
            traversal.pop();
            ...
            if (node == nullptr) {
                ...
                std::cout << std::setw(2) << std::right << std::setfill(' ') << node->value << '|' << node->height;
...
// numbers.
void ugly_print_tree(const AVL<int>& tree) {
    const AVL<int>::Node* root = tree.getRootNode();
    if (root == nullptr) {
        std::cout << "Empty tree" << std::endl;
    } else {
